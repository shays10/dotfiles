...
4. Make the smallest fix at the root cause.
...
4. Implement the smallest complete change. Avoid compatibility layers that exist only to smooth an internal migration.
5. Run focused tests, required linting, and a check against the real artifact when available.

Finish when the requested behavior is observable and the diff contains no unrelated cleanup.
